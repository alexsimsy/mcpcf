...
export async function checkRateLimit(
    ...
    const rateLimitData = await env.RATE_LIMIT_KV.get<RateLimitData>(key, 'json');
...
export async function checkIPBlock(
    ...
    const blockData = await env.IP_BLOCK_KV.get<BlockData>(key, 'json');
    const now = Math.floor(Date.now() / 1000);
...
export async function trackFailedAttempt(
    ...
    const failedAttempts = await env.IP_BLOCK_KV.get<number>(key, 'json') || 0;
